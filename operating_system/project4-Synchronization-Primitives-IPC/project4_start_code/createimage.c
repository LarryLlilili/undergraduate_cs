...
{
	int kernel_size = 0x26000+task_phdr->p_filesz;  
...
int main(int argc, char *argv[]){
	...
	FILE *pro1_file = NULL, *pro2_file=NULL, *pro3_file=NULL, *pro4_file=NULL;
	Elf32_Ehdr *boot_ehdr = NULL, *kernel_ehdr = NULL;
	Elf32_Ehdr *pro1_ehdr = NULL, *pro2_ehdr = NULL, *pro3_ehdr=NULL, *pro4_ehdr=NULL; 
	Elf32_Phdr *boot_phdr = NULL, *kernel_phdr = NULL;
	Elf32_Phdr *pro1_phdr = NULL, *pro2_phdr = NULL, *pro3_phdr=NULL, *pro4_phdr=NULL;
    ...
    fread(pro4_buf, 4096, 1, pro4_file);
	...
	pro4_ehdr = (void*) pro4_buf;
	pro4_phdr = (Elf32_Phdr*)((void*)pro4_buf + pro4_ehdr->e_phoff);

	write_bootblock(&image_file, boot_file, boot_ehdr, boot_phdr);
	write_kernel(&image_file, kernel_file, kernel_ehdr, kernel_phdr);   
	write_task(&image_file, pro1_file, pro1_ehdr, pro1_phdr, 0x20000);
	write_task(&image_file, pro2_file, pro2_ehdr, pro2_phdr, 0x22000);
	write_task(&image_file, pro3_file, pro3_ehdr, pro3_phdr, 0x24000);
	write_task(&image_file, pro4_file, pro4_ehdr, pro4_phdr, 0x26000);

	 //number of sector of kernel
 	//change bootblock
	record_kernel_sectors(&image_file, pro4_ehdr, pro4_phdr);
	...
	printf("\tSIZE: %d\n",pro4_phdr->p_filesz);
	printf("\tSEC NUMBER: %d\n",(pro4_phdr->p_filesz)/512+(pro4_phdr->p_filesz % 512 != 0));   //////////
	printf("\tOFFSET in file: %d\n",pro4_phdr->p_offset);
	printf("\tOFFSET in memory: 0x%x\n",pro4_phdr->p_vaddr);
	printf("\tSIZE in file:  %d\n",pro4_phdr->p_filesz);
	printf("\tSIZE in memory: %d\n",pro4_phdr->p_memsz);
